    ...
    <div className="space-y-5 animate-fade-in">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0">
          {cats.map((cat) => (
            <Link
              ...
              className={`inline-flex items-center px-3 py-1.5 rounded-full text-caption font-medium whitespace-nowrap transition-all ${
                categoryId === cat.id ? 'ring-2 ring-offset-1 ring-ink-400' : 'hover:opacity-80'
              }`}
              style={{ backgroundColor: cat.color, color: '#35332E' }}
            >
              {cat.name}
            </Link>
          ))}
      ...
      {list.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <p className="text-body text-ink-700 mb-1">沒有符合條件的產品</p>
...
function FilterPill({ href, label, active }: { href: string; label: string; active: boolean }) {
  return (
    <Link
      href={href}
      className={`inline-flex items-center px-3.5 py-1.5 rounded-full text-caption font-medium whitespace-nowrap transition-colors ${
        active
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-body font-medium text-ink-900 truncate">{product.name}</span>
          {product.status === 'in_use' && (
            <span className="text-micro px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded flex-shrink-0">使用中</span>
          )}
          {product.status === 'unopened' && (
            <span className="text-micro px-1.5 py-0.5 bg-ink-100 text-ink-600 rounded flex-shrink-0">未開封</span>
          )}
        </div>
